...
#1 input_url = 'https://news.mit.edu/2025/device-enables-direct-communication-among-multiple-quantum-processors-0321'
input_url='https://observer.com/2025/06/future-of-quantum-computing-innovation/'
async def async_main():
    conn = sqlite3.connect("quantum_news.db")
    cursor = conn.cursor()

    # ✅ Ensure the table exists with a 'date' column
    cursor.execute("""
    ...
    try:
        ...
        # ✅ Insert into the database with a timestamp
        cursor.execute("INSERT INTO news (title, url, summary, date) VALUES (?, ?, ?, CURRENT_TIMESTAMP)", 
                       (title, input_url, summary))
        conn.commit()
